...
export class TrainSystem {
  ...
  createRail(curve, name = false){
    ...
    for(let i = 0; i<points.length; i++){
      const pos = points[i]
      this.object_update({ins_obj: sleeper, ins_idx: i, pos_x: pos.x,  pos_y: pos.y-0.9, pos_z: pos.z, rot_x: NaN, rot_y: angles[i], rot_z: NaN,scale: NaN})
      const x_sin = Math.sin(angles[i])
      const z_cos = Math.cos(angles[i])
      this.object_update({ins_obj: loc, ins_idx: i*2, pos_x: pos.x+x_sin*0.245,  pos_y: pos.y-0.86, pos_z: pos.z+z_cos*0.21, rot_x: NaN, rot_y: angles[i], rot_z: NaN,scale: NaN})
